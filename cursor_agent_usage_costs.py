...
def filter_rows_window(
    ...
    *,
    ...
    return [row for row in rows if in_window(row.timestamp, start, end, inclusive_end=inclusive_end)]
...
def selected_cursor_token_breakdown(summary: dict[str, Any]) -> dict[str, int]:
    ...
    return {
        ...
        "cache_write": int(totals["cache_write_tokens"]),
    }


def build_mode_payload(rows: list[UsageRow], now: datetime, mode: str) -> dict[str, Any]:
    buckets, config = bucket_range(now, mode)
    selected_label, selected_start, selected_end = selected_window(now, mode)
    selected = summarize_rows(
        filter_rows_window(rows, selected_start, selected_end, inclusive_end=True)
    )
    bucket_summaries = [
        {
    ...
    return {
        "selected": {
            ...
            "cost_usd": selected["totals"]["reported_cost_usd"],
            "token_breakdown": selected_cursor_token_breakdown(selected),
            "events": int(selected["totals"]["events"]),
        },
        "graph": {
            "mode": mode,
...
def build_all_modes_payload(rows: list[UsageRow], now: datetime | None = None) -> dict[str, Any]:
    if now is None:
        now = datetime.now().astimezone()
    relevant_rows = filter_rows_window(rows, all_modes_start(now), now, inclusive_end=True)
    return {
        "updated_at": int(now.timestamp()),
        "modes": {
            mode: build_mode_payload(relevant_rows, now, mode) for mode in GRAPH_MODES
        },
    }
...
def main() -> int:
    ...
    if args.all_modes:
        now = datetime.now().astimezone()
        filtered_rows = filter_rows(all_rows, args)
        payload = {
            "source": source,
            ...
            **build_all_modes_payload(filtered_rows, now),
        }
